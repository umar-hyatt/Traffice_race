...
public class RCC_CarControllerV3 : MonoBehaviour {
	...
	public float brakeTorque = 5000f;							// Maximum Brake Torque.,
	...
	public bool useRevLimiter = false;								// Rev Limiter above Maximum Engine RPM.
	public bool useExhaustFlame = false;						// Exhaust blow flame.
	public bool useClutchMarginAtFirstGear = false;		// Smooth clutching at first gear.
	public float steerAngle = 20f;									// Maximum Steer Angle Of Your Vehicle.
	public float highspeedsteerAngle = 15f;					// Maximum Steer Angle At Highest Speed.
	public float highspeedsteerAngleAtspeed = 250f;		// Highest Speed For Maximum Steer Angle.
	...
	public AudioClip sirenClip{get{return HR_HighwayRacerProperties.Instance.sirenAudioClip;}}
	...
	[HideInInspector]public float minGasInput = 0f;
	...
	internal float _gasInput{get{
			...
			if(!automaticGear || semiAutomaticGear){
				if(!changingGear && !cutGas)
					return Mathf.Clamp01(gasInput + minGasInput);
				else
					return 0f;
			}else{
				if(!changingGear && !cutGas)
					return (direction == 1 ? Mathf.Clamp01(gasInput + minGasInput) : Mathf.Clamp01(brakeInput));
				else
					return 0f;
			}
	...
	internal float _steerInput{get{
		...
		}set{steerInput = value;}}

	internal float _verticalAngle{get{

			return verticalAngle;

		}set{verticalAngle = value;}}

	internal float _handbrakeInput{get{
	...
	public bool applyCounterSteering = false;							// Applies counter steering when vehicle is drifting. It helps to keep the control fine of the vehicle.
	...
	// HIGHWAY RACER
	public HR_RoadPooling roadPooling;
	private float minimumSpeed = 20f;

	private bool useHelpersForHighway = true;
	public float highwaySteeringHelper = 4f;
	public float highwayBrakingHelper = 4f;
	internal float verticalAngle = 0f;
...
//		GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(10f, 1, 1);
		...
		case RCC_Settings.BehaviorType.HighwayRacer:
			...
			antiRollFrontHorizontal = Mathf.Clamp (antiRollFrontHorizontal, 5000f, Mathf.Infinity);
			antiRollRearHorizontal = Mathf.Clamp (antiRollRearHorizontal, 5000f, Mathf.Infinity);
			steerAngle = Mathf.Clamp (steerAngle, 10f, 40f);
			applyCounterSteering = false;
			rigid.drag = Mathf.Clamp (rigid.drag, .025f, Mathf.Infinity);
			rigid.angularDrag = Mathf.Clamp (rigid.angularDrag, 5f, Mathf.Infinity);
			//			highspeedsteerAngle = 10f;
			//			highspeedsteerAngleAtspeed = 80f;
			break;
	...
	void SoundsInitialize (){
		...
		hornSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "Horn/Siren Sound AudioSource", 0, 0, 1, PlayerPrefsX.GetBool(transform.name + "SirenAttached", false) ? sirenClip : hornClip, true, false, false);
	...
	void CheckRoadWidth(){
		...
		if(HR_GamePlayHandler.Instance && !HR_GamePlayHandler.Instance.gameStarted)
			return;

		roadAngle = Mathf.Clamp (Quaternion.Angle (transform.rotation, Quaternion.identity) / (transform.eulerAngles.y > 0 && transform.eulerAngles.y < 180 ? -30f : 30f), -1f, 1f);

		if(speed < 10f || Mathf.Abs(transform.position.x) > 10f || Mathf.Abs(transform.position.y) > 10f){
			
			transform.position = new Vector3(0f, 2f, transform.position.z + 10f);
			transform.rotation = Quaternion.identity;
			rigid.angularVelocity = Vector3.zero;
			rigid.velocity = new Vector3(0f, 0f, minimumSpeed / 2f);
...
//			handbrakeInput = 1f;
		...
		float angle = Mathf.Clamp (Quaternion.Angle (transform.rotation, Quaternion.identity) / (transform.eulerAngles.y >= 0 && transform.eulerAngles.y <= 180 ? -20f : 20f), -1f, 1f);
		verticalAngle = angle * 1f;
	...
	void FixedUpdate (){
		...
		if (useHelpersForHighway && roadPooling) {

			if(speed > minimumSpeed)
				rigid.AddForce (-Vector3.forward * (brakeInput * highwayBrakingHelper * 1f), ForceMode.Acceleration);

		}

		rigid.centerOfMass = transform.InverseTransformPoint(COM.transform.position);

		steerHelperLinearVelStrength = Mathf.Lerp(-.3f, 1f, highwaySteeringHelper / 10f);
		steerHelperAngularVelStrength = steerHelperLinearVelStrength;
	...
	void SteerHelper(){
		...
		velocityDirection.localRotation = Quaternion.Lerp(velocityDirection.localRotation, Quaternion.AngleAxis(Mathf.Clamp(velocityAngle / 1f, -100f, 100f), Vector3.up), Time.fixedDeltaTime * 20f);
		...
		rigid.AddRelativeTorque (Vector3.up * ((angle2 * (Mathf.Clamp(transform.InverseTransformDirection(rigid.velocity).z, -200f, 200f) / 10000f)) * steerHelperAngularVelStrength), ForceMode.VelocityChange);
	...
	void Clutch(){
		...
		if (engineRunning) {
			...
			minGasInput = Mathf.Clamp (Mathf.Lerp (3f, 0f, speed / minimumSpeed), 0f, 1f);
		} else {
			idleInput = 0f;
			minGasInput = 0f;
		}
	...
	public void GetControllerType(){
		...
		if(PlayerPrefs.GetInt("ControllerType") == 0)
			RCCSettings.mobileController = RCC_Settings.MobileController.TouchScreen;
		else
			RCCSettings.mobileController = RCC_Settings.MobileController.Gyro;
