...
public class RCC_Customization : MonoBehaviour {
	...
	/// </summary>
	public static void UpdateRCC (RCC_CarControllerV3 vehicle) {
	...
	public static void ChangeWheels(RCC_CarControllerV3 vehicle, GameObject wheel){
		...
		RCC_WheelCollider[] allWheelColliders = vehicle.GetComponentsInChildren<RCC_WheelCollider> ();

		for (int i = 0; i < allWheelColliders.Length; i++) {

			if (allWheelColliders [i].wheelModel.GetComponent<MeshRenderer> ()) 
				allWheelColliders [i].wheelModel.GetComponent<MeshRenderer> ().enabled = false;

			foreach (Transform t in allWheelColliders [i].wheelModel.GetComponentInChildren<Transform> ())
				t.gameObject.SetActive (false);

			GameObject newWheel = (GameObject)Instantiate (wheel, allWheelColliders[i].wheelModel.position, allWheelColliders[i].wheelModel.rotation, allWheelColliders[i].wheelModel);

			if (allWheelColliders [i].wheelModel.localPosition.x > 0f)
