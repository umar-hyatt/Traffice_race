...
public class RCC_MobileButtons : MonoBehaviour {
	...
	public RCC_UIController headLightsButton;
	...
	void Start(){
		...
		if(RCCSettings.controllerType != RCC_Settings.ControllerType.Mobile){
			...
			if(headLightsButton)
				headLightsButton.gameObject.SetActive(false);
	...
	void Update(){
		...
		if (RCC_SceneManager.Instance.activePlayerVehicle) {
			...
			if(NOSButton && NOSButton.gameObject.activeInHierarchy != RCC_SceneManager.Instance.activePlayerVehicle.useNOS)
				NOSButton.gameObject.SetActive(RCC_SceneManager.Instance.activePlayerVehicle.useNOS);
			...
			if (RCC_SceneManager.Instance.activePlayerVehicle.canControl && !RCC_SceneManager.Instance.activePlayerVehicle.externalController) {
				...
				RCC_SceneManager.Instance.activePlayerVehicle.gasInput = Mathf.Clamp01(gasInput + (NOSInput - 1f));
				RCC_SceneManager.Instance.activePlayerVehicle.brakeInput = brakeInput;
				RCC_SceneManager.Instance.activePlayerVehicle.steerInput = -leftInput + rightInput + steeringWheelInput + gyroInput;
				...
				RCC_SceneManager.Instance.activePlayerVehicle.highBeamHeadLightsOn = headLightsButton.pressing ? true : false;
