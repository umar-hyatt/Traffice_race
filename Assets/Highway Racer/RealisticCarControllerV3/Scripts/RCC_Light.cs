...
public class RCC_Light : MonoBehaviour {
	...
	void Start () {
		...
		if(RCCSettings.useLightProjectorForLightingEffect && lightType == LightType.HeadLight){
...
//			_light.renderMode = LightRenderMode.ForcePixel;
	...
	void Update () {
		...
		case LightType.HeadLight:
			if (highBeamLightFound) {
				...
				Lighting (carController.lowBeamHeadLightsOn ? .6f : 0f, 50f, 90f);
		...
		case LightType.ParkLight:
			Lighting((!carController.lowBeamHeadLightsOn ? 0f : .25f));
			break;
	...
	private void LensFlare(){
		...
		if(angle != 0)
			finalFlareBrightness = flareBrightness * (4 / distanceTocam) * ((100f - (1.11f * angle)) / 100f) / 3f;
